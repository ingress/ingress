...
describe('Node.js HTTP Integration', () => {
  ...
  describe('Client Error Handling', () => {
    ...
    it('should handle client errors with default handler when no custom handler is provided', async () => {
      ...
      // The specific error varies by Node.js version and timing, but we should get some error
  ...
  describe('Multiple HTTP Instances', () => {
    it('should share the same server instance when using nested apps', async () => {
      ...
      // Nest app B within app A
      appA.use(appB)
      await appA.run()

      // Both HTTP instances should share the same underlying server
      assert.strictEqual(
   ...
   * Used for coordinating async test completion
   */
  ...
  /**
   * Sends a malformed HTTP request to trigger client error handling
   * @param serverAddress - The address info of the server to connect to
   * @returns Promise that resolves with any error that occurs
   */
  async function sendMalformedHttpRequest(serverAddress: AddressInfo) {
    ...
    return new Promise((resolve) => {
      const connection = createConnection(port, address, () => {
        ...
        // Send a malformed HTTP request that will trigger a parse error
        // Invalid method with null byte and missing space between method and path
        connection.write('INVALID_METHOD\x01/path HTTP/1.1\r\n\r\n')

        // Clean up connection after allowing time for server processing
        setTimeout(() => {
