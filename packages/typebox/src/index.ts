...
class BoundaryTypeBuilder extends JavaScriptTypeBuilder {
  public Boundary<T extends TSchema>(
    schema: T,
    parse?: (raw: unknown, check: TypeCheck<T>, schema: T) => StaticDecode<T> | Promise<StaticDecode<T>>,
    pick?: Annotation,
    ...
    const compiled = TypeCompiler.Compile(schema),
      annotated = schema as T & {
        ...
        [kIngressRouterParserKind]: 'typebox:v1'
      }
    if (pick) {
    ...
    annotated[kIngressRouterParse] = parse
      ...
      : (pick?.annotationInstance?.[kIngressRouterParse]?.bind(pick.annotationInstance) ??
        function (raw: any) {
          ...
          throw new ING_BAD_REQUEST(error && `${error.message}${error.path ? ` at ${error.path}` : ''}`)
        })
